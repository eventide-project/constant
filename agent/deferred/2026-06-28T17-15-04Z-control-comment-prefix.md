...
**Scope:** the `Constant`-class tests under `test/automated/constant/` (build/, namespace/, name/, full_name, equality/). The `import_constant/` tests already use the "Control ..." form in places; reconcile for consistency if conformed. This pairs with the namespace-variable-suffix reconciliation and the broader control-naming conforming.
