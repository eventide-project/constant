...
`include Initializer` and declares `initializer :value`. This yields a
`Constant.new(value)` that records the value (`@value = value`) and a `#value`
reader — nothing else: no validation, no deconstruction. Passing an invalid
...
The reader generated by `initializer :value`, returning the recorded
module/class. It is reader-only — no writer is generated.
