...
**Where it lands.** What survives is the *unavailable options*, and that is not a new thing: **the option the AI can't produce, the decision it can't recognize as needing the person, and the answer it can't predict are the same single thing seen three ways — what only the person can produce.** Fault-line A doesn't get closed; it gets **handed to the person**, which is what you do with anything the AI can't produce. It stays with the person permanently, because it's about options the AI can never produce — not about the quality of its average — so improving the AI doesn't touch it.
