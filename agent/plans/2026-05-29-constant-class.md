...
**Goal:** Add a stateful `Constant` class to the `constant` library that mediates a resolved constant object (a module or class) and answers queries about its name, namespace, presence, and inner constants.
...
Built and reworked: `#namespace` returns the containing constant **as a `Constant`** (not the bare module, and not the namespace name as a String — both forms were tried and dropped). A top-level constant's namespace is `Constant.new(Object)` — a method that must return a `Constant` can't return `nil`, so `Object` is the top-level fixpoint.
...
- [x] **Value equality** — `Constant#==`, `#eql?`, and `#hash` compare by the mediated `raw_constant`: equal `Constant`s compare equal, dedupe in a `Set`, and interchange as `Hash` keys. A non-`Constant` operand compares `false` without raising.

---

## Task 5: Build from a constant object

- [x] Given a constant object, `build` returns the `Constant` that mediates it.

---

## Task 6: Build from a name

- [x] Given a name and a namespace, `build` resolves the name to its constant object and returns the `Constant` that mediates it. The namespace defaults to the top level. An option directs whether resolution follows the ancestor chain. When the name is not bound in the namespace, or is bound to a value that is not a constant object, `build` raises an error.

Driven outcomes (red→green): name resolved within a namespace; `Constant::Error` on an undefined name; `Constant::Error` on a name that resolves to a non-module value. The **default-namespace** and **`inherit`** outcomes were *dropped as green-on-arrival* — both behaviors were already present (the `namespace = Object` default and `const_get`'s threaded `inherit`), so their tests would have driven no design (see the no-green-on-arrival-tests rule). Extension deferred: `build`'s namespace parameter accepting a name as well as a raw constant (`agent/deferred/2026-06-28T15-57-24Z-build-namespace-as-name.md`).
...
- [ ] A `Constant` reports its own inner constants that are themselves constant objects, each mediated by a `Constant`. The selection matches the names query; the difference is that the mediating `Constant`s are returned rather than name Strings.
