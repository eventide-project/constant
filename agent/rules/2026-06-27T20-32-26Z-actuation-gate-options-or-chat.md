...
- **The escape is mandatory (fault-line A — options the AI can't produce).** The option set is the AI's own, drawn from the average; the best actuation is often one only the human can produce, and showing a closed list tempts the human to pick from it instead of producing their own. "Chat about this" / dictate is the origination escape — the human is never boxed into the AI's option set.
