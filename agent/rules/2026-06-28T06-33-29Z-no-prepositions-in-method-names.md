...
**Scope — method names only.** This governs **method/initializer names**, not prose or terminology. Prose deliberately *does* use prepositional phrasing where it carries meaning — e.g. inputs are described as **"sent to"** a method (`agent/rules/2026-06-05T17-38-24Z-sent-to-phrasing.md`). That is a description of a relationship, not an identifier; the preposition is load-bearing there and absent here. (The relation verb is now the bare **"mediates"** — see the no-slang terminology rule — so it is no longer an example of load-bearing prepositional prose.)
