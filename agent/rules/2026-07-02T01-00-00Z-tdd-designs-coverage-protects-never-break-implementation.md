...
- **TDD is a design activity.** The failing-test-then-implement cycle is an
  ...
  red is what drives the implementation into existence.
...
already exists, so the test passes immediately. That green *is the point* — the
behavior is now protected. It is the nature of coverage, not a defect to engineer
around, and the no-green-on-arrival stance is a TDD-*design* concern that does not
apply here.
...
keeps TDD honest (tests as design proofs) and coverage honest (tests as regression
...
write the test, run it, accept the green, and leave the implementation untouched.
Reserve the failing-test-first cycle for TDD, where a test drives new design.
Related: the companion rule "the hinges gate the design of a test" (they still
apply when writing coverage), the TDD-as-design-tool rule, and the hinge-cycle
rule.
