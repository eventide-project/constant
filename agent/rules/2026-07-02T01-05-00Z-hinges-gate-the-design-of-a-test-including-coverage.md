...
The five hinges (actuation, assertion/observation, controls, implementation,
naming) gate **design decisions**, and *designing a test* is itself a design act —
independent of whether behavior is being designed (TDD) or existing behavior is
being covered (testing). So the hinges are in play when writing coverage, not only
when doing TDD.

When adding coverage for behavior that already exists, all five are in play:

- **Actuation** — confirm the call's shape.
...
- **Observation (assertion / test block)** — a real decision: what to assert and
  how to read the behavior's effect.
...
and observations — so it earns the same gating. What separates coverage from TDD is
...
implementation hinge as a confirmation step (run, show green), sending any
awkwardness back to the test, not the implementation. Related: the hinge-cycle rule
and the companion "TDD designs, coverage protects" rule.
