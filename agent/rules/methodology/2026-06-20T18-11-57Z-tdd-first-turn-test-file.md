...
**Acceptance is separate gates, not one.** The *content* of turn one is as above, but the human accepts it in separate hinges — **actuation, then assertion, then controls** (then the implementation as the fourth hinge, and the naming as the fifth, at the feature's close) — each proposed and accepted before the next, per the hinge-cycle rule (`agent/rules/methodology/2026-06-28T15-04-47Z-hinge-cycle.md`). Do not present the whole test file for a single accept; settle the three test hinges first, then assemble and show the file. Related: the TDD-as-design-tool rule, the test-structure rule (the grown form), the folder-mirroring rule, the actuation-gate options-or-chat rule, and the `control_` test-variable prefix rule.
