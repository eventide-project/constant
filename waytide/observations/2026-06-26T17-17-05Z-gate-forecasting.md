...
## Open questions (two limits on Law 1)

**Terminology (2026-06-27):** these were called "fault-lines"; the geological metaphor was retired for the plain term **limit** (per the name-literally principle). A *limit* is a condition under which a principle stops fully holding — surfaced and tracked as an open question, and classifiable by whether it is permanent (limit A: survives a better AI) or self-dissolving (limit B: goes away as conditioning matures). The discriminator-era "soft spots" are the same kind of thing but a distinct labeled pair, left under their own name to avoid a "limit A/B" collision across the two frameworks.

Both are limits on **Law 1** (*provoke origination, not mean-bias*): one attacks its mitigation, one its applicability.

### Limit A — options the AI can't produce
...
It looks like a 2×2 — reachable / not × surfaced / not — but the fourth cell is **impossible**: being reached presupposes being reachable (you can't surface what you can't produce), so *unreachable options are always off the list*. That collapses the grid to these three rungs. Limit A lives in rungs **2 and 3** — the options absent from the list.
...
**Two permanent taxes (no free lunch).** (1) **Blind-audit sampling** — you pay an origination cost forever to keep the estimate fresh and catch drift; confidence is never free. (2) **Seam vigilance** — limit A means even perfect confidence on what the AI *can* produce never covers what only the person can produce, so catch-the-seam vigilance is also permanent. Conditioning confidence can relax the laws on what the AI can see; it can do nothing about what it can't.
